...
_EOF_
    ...
    sed -i "s/route = route = 192.168.0.0/16/#route = 192.168.0.0/16/g" "${confdir}/ocserv.conf"
    sed -i "s/dns = 1.1.1.1/dns = 8.8.4.4/g" "${confdir}/ocserv.conf"
    sed -i "s/server-cert = /etc/ssl/certs/ssl-cert-snakeoil.pem/server-cert = /etc/letsencrypt/live/vpn.example.com/fullchain.pem/g" "${confdir}/ocserv.conf"
    sed -i "s/server-key = /etc/ssl/private/ssl-cert-snakeoil.key/server-key = /etc/letsencrypt/live/vpn.example.com/privkey.pem/g" "${confdir}/ocserv.conf"
    sed -i "s/try-mtu-discovery = false/try-mtu-discovery = true/g" "${confdir}/ocserv.conf"
    sed -i "s/#tunnel-all-dns = true/tunnel-all-dns = true/g" "${confdir}/ocserv.conf"
    sed -i "s/device = vpns/device = device-/g" "${confdir}/ocserv.conf"
